...
ci_random = random.Random()
ci_random.seed(random_seed)


with open(r'/home/work/chanwoo/ml-flair/data/sorted_state_known_by_data_num.pkl', 'rb') as f: 
    sorted_state_known_by_data_num = pickle.load(f)

with open(r'/home/work/chanwoo/ml-flair/data/sorted_state_known_ci_60percent_by_datanum.pkl', 'rb') as f: 
    sorted_state_known_by_data_num_60percent = pickle.load(f)

with open(r"/home/work/chanwoo/ml-flair/data/state_known_user_data.json", "r") as f:
    user_data = json.load(f)

with open(r'/home/work/chanwoo/ml-flair/data/low_ci.pkl', 'rb') as f: 
    low_ci = pickle.load(f)

with open(r'/home/work/chanwoo/ml-flair/data/mid_ci.pkl', 'rb') as f: 
    mid_ci = pickle.load(f)

with open(r'/home/work/chanwoo/ml-flair/data/high_ci.pkl', 'rb') as f: 
    high_ci = pickle.load(f)
...
flags.DEFINE_integer('client_batch_size', 8, 'Batch size on the clients.') # pixel XL's maximum batchsize=4
flags.DEFINE_integer('clients_per_round', 40, #200
...
def main(argv):
    ...
    def choose_client_id_with_probability(user_data: dict, user_id_lst: list, K: int):
    ...
    while round_num < FLAGS.total_rounds:
        ...
        # if choice <=335/3050:
        #     client_cluster = list(sorted_state_known_by_data_num)[:335]
        # else:
        #     client_cluster = list(sorted_state_known_by_data_num)[335:]
        ...
        # if round_num % 10 == 0:
        #     cur_random = cluster_random.randint(0,9)
        ...
        # sampled_clients = choose_client_id_with_probability(user_data, client_cluster, 50)
        # sampled_clients = tool.choose_client_id_with_credits(user_data, user_credits, sorted_state_known_by_data_num, 50, 1)

        # ci 60%
        # sampled_clients = random.sample(sorted_state_known_by_data_num_60percent, 50)

        # data heterogeneity
        # sampled_clients = tool.choose_client_id_greedy(sorted_state_known_by_data_num, round_num)

        # FedAdam
        # sampled_clients = random.sample(sorted_state_known_by_data_num, 50)

        # ci background 실험
        # sampled_clients = ci_random.sample(low_ci + mid_ci + high_ci, 40)
        # sampled_clients = ci_random.sample(low_ci, 14 if round_num%3==0 else 13) + \
        #                   ci_random.sample(mid_ci, 14 if round_num%3==1 else 13) + \
        #                   ci_random.sample(high_ci, 14 if round_num%3==2 else 13)
        sampled_clients = ci_random.sample(low_ci, 20) + ci_random.sample(mid_ci, 10) + ci_random.sample(high_ci, 10)
