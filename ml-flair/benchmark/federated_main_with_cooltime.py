...
flags.DEFINE_integer('client_datasets_random_seed', 999, # 내가 작성
...
def main(argv):
    ...
    user_cooltime = tool.make_user_cooltime(user_data) # user cooltime dict
    ...
    while round_num < FLAGS.total_rounds:
        ...
        sampled_clients = tool.choose_client_id_with_cooltime(user_data, user_cooltime, client_cluster, 50, 355/2)

        metrics = {'prepare datasets secs': time.time() - data_prep_start_time}

        state, round_metrics = training_process.next(state, list(sampled_clients))
