...
def EfficientNet(
    ...
    x = tfa_norms.GroupNormalization(
        axis=bn_axis, epsilon=1e-3, name="Conv/BatchNorm"
    )(x)
    ...
    x = tfa_norms.GroupNormalization(axis=bn_axis, epsilon=1e-3, name="top_bn")(x)
...
def block(
    ...
    if expand_ratio != 1:
        ...
        x = tfa_norms.GroupNormalization(axis=bn_axis, epsilon=1e-3, name=name + "expand_bn",
                                         groups=24 if x.shape[-1]==144 or x.shape[-1]==240 else 32)(x)
    ...
    x = tfa_norms.GroupNormalization(axis=bn_axis, epsilon=1e-3, name=name + "bn",
                                     groups=24 if x.shape[-1]==144 or x.shape[-1]==240 else 32)(x)
    ...
    groups = -1
    if x.shape[-1] == 16 or x.shape[-1] == 112:
        groups = 16
    elif x.shape[-1] == 24:
        groups = 24
    elif x.shape[-1] == 40 or x.shape[-1] == 80:
        groups = 20
    else:
        groups = 32

    x = tfa_norms.GroupNormalization(axis=bn_axis, epsilon=1e-3, groups=groups, name=name + "project_bn")(x)
